...
Gates Protocol** (`protocols/quality-gates.md`) on an artifact and returns a scored, auditable
...
- Its **context**: persona, grade band, subject, the standards it claims to meet, and the original
  ...
  (`protocols/assumptions-protocol.md`) — never invent facts about the artifact.
...
Return the verdict as a decision record per `protocols/metadata-schema.md`: per-dimension scores +
...
contradictions with `protocols/conflict-protocol.md` (integrity > safety > governance >
accuracy/alignment > preference > presentation). Never fabricate to make something pass
(`protocols/failure-recovery.md`).
