...
boundaries, and regression expectations for `document-intelligence` so a future maintainer keeps the intended control
plane instead of patching symptoms. Canonical procedure: `tools/skill-maintenance.md`.

`document-intelligence` is turn documents (PDF/DOCX/HTML/TXT/images/scans/Google Workspace) into GOVERNED knowledge assets via a parser-independent, artifact-centric, fully OFFLINE pipeline.
...
Skill-specific:
- fully OFFLINE at run time — no network; optional engines (PyMuPDF/pdfplumber/Tesseract) activate only when installed
- parser-independent: parsers/OCR engines are swappable plugins behind the UDOM contract
- every artifact carries provenance/lineage/confidence; NEVER fabricate — unrecovered regions are reported at low confidence, never invented
- not an authoring tool, not a web/Drive crawler, not a search engine

## Known failure modes
- inventing content for an unreadable region
- presenting metadata-only retrieval as ingested content
- requiring network access at run time
- merging multi-paragraph table cells and losing structure

## Fragile fallbacks that must not become defaults
- low-confidence OCR must stay labeled — never silently 'cleaned' into apparent certainty
- the retrieval-state ladder must be honored: referenced → metadata_only → content_ingested → local_artifact_saved

## Regression cases to preserve
1. an image-only PDF is OCR'd with a confidence score and no fabrication
2. the pipeline runs with no network available
3. an unreadable region produces a gap, not invented text
...
Skill-specific:
- the UDOM / plugin contracts
- the offline guarantee
- the retrieval-state ladder
...
5. skill-specific:
   - runs offline
   - confidence/provenance are present
   - no fabrication of unrecovered content
