...
- **Required elements:** per-dimension 0-5 scores *with quoted evidence*; composite; decision
  ...
  rationale; `human_review_required: true`. Schema: `protocol-layer/metadata-schema.md`.
...
- **Required elements:** for each gap — what's wrong (with evidence), why it matters (which gate),
  and the concrete change to make. Follows `protocol-layer/failure-recovery.md` (never fabricate to pass)
  and `protocol-layer/conflict-protocol.md` (authority hierarchy) where requirements conflict.
