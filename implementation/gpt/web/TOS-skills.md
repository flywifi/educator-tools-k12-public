...
**On ChatGPT:** ChatGPT cannot run the TOS profile script — follow "The Setup Wizard — start here" at the top of this document instead: a one-question-at-a-time interview that explains why it asks each thing, then saves the answers as my-teacher-profile.md in this Project. Any chat that can see that file treats it as the registered profile (teacher-stated facts still outrank everything else). Placeholders only — never real student names.
