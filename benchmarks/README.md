...
## The two fights (why this is phased)

"Document-related" is really two different contests, and honesty requires naming which one a claim
belongs to:

- **Generation grounded in verified standards + governance** — where TOS genuinely wins and can
  prove it. This is **Phase A** (axes 1–6), run now.
- **Raw ingestion / parsing** (PDF tables, OCR, layout) — a contest against purpose-built engines
  (Docling, Marker, Unstructured) that publish leaderboard numbers. TOS's edge here is
  provenance + i18n-honesty + capability-gap honesty, not raw-extraction supremacy. This is
  **Phase B** (axis 7), and its honest goal is *parity + honesty edge*, potentially by composing a
  best-in-class engine under TOS governance — not a supremacy claim.
...
| `oss_parsers` | Docling / Marker / Unstructured (Phase B, ingestion axis) | Scriptable if installed; else public-leaderboard citation with source |
...
| # | Axis | Objective grader | Phase |
|---|---|---|---|
| 1 | **Standards-grounding fidelity** *(headline)* | every standard/course code an output cites is resolved against `tools/offline_index.py`; count fabricated (count=0), miscoded, and version-missing. Impossible asks (Hogwarts; grade-1-math photosynthesis) must return *empty*, not an invented code. | A |
| 2 | **Governance / auditability** | presence of a gated decision record + deterministic composite (`score.py`) + the fabricated-`3.NF.A.9` critical-failure catch + `human_review_required`. | A |
| 3 | **Generation quality** | blind 9-dimension rubric composite via `score.py` (judge assigns dims with quoted evidence per `quality-review/references/rubric.md`). | A |
| 4 | **Differentiation consistency** | correct, standards-aligned ELL / IEP / gifted tiers present and internally consistent. | A |
| 5 | **Output / format fidelity** | real `.pptx/.docx/.xlsx/PDF` structural validity via `tools/validate_document.py`; Google-bridge manifest. | A |
| 6 | **Cost / efficiency** | tokens for grounded lookup vs. loading the corpus (the measured ~99.93% reduction, `docs/END_TO_END_WALKTHROUGH.md`). | A |
| 7 | **Ingestion / parsing** | text-recovery, table-fidelity (TEDS/GriTS-style), i18n-preservation, container recursion, and **retrieval_state honesty** (visibility ≠ extraction). | B |
...
- **Axis 7 (Phase B):** the honest target is **parity on raw accuracy + a measured honesty edge**
...
benchmarks/
  ...
  corpus/            # (Phase B) adversarial ingestion documents
```
