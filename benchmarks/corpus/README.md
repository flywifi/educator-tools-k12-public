# corpus/ — adversarial ingestion documents (Phase B)

Phase B (axis 7, parsing) needs documents that exercise docintel's harder code paths, which the
...
*(Empty until Phase B. Synthesize the adversarial files here with a small generator so they're
reproducible rather than opaque binaries checked in by hand.)*
