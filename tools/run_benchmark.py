#!/usr/bin/env python3
"""run_benchmark.py — governed document benchmark harness (Phase A).
...
def report() -> int:
    ...
    lines = [
        ...
        "",
        f"Arms: {', '.join(ARMS)} · OSS parsers (ingestion) are Phase B.",
        "",
        "## Per-task summary",
        "",
    ]
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="Governed document benchmark harness (Phase A).")
