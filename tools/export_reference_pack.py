...
ONEFILE = "tos-reference-pack-onefile.json"
ZIPFILE_NAME = "reference-pack.zip"
# fixed timestamp so the zip is byte-reproducible across rebuilds
...
def _onefile(spec: list[dict], manifest_text: str) -> str:
    """The whole pack as ONE JSON file — for Free-tier Projects (~5 file slots) and
    two-download setups. Same data; per-file provenance preserved under named keys."""
    out = {
        "_comment": "GENERATED single-file Reference Pack (all pack data under named keys; "
                    f"receipts in _manifest). Built by tools/export_reference_pack.py — never "
                    f"edit by hand. Project source: {REPO_URL}",
        "_manifest": manifest_text,
        "human_review_required": True,
    }
    for e in spec:
        key = e["dest"].rsplit(".", 1)[0].replace("-", "_")
        if isinstance(e["src"], list):
            out[key] = _build_frameworks()
        else:
            out[key] = _load(e["src"])
    return json.dumps(out, indent=2, ensure_ascii=False) + "\n"


def _zip_bytes(spec: list[dict]) -> bytes:
    """Reproducible zip of the individual pack files (fixed timestamps, sorted names)."""
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
        for name in sorted([e["dest"] for e in spec] + ["MANIFEST.md"]):
            info = zipfile.ZipInfo(f"reference-pack/{name}", date_time=_ZIP_DATE)
            info.compress_type = zipfile.ZIP_DEFLATED
            z.writestr(info, (PACK_DIR / name).read_bytes())
    return buf.getvalue()
...
def _manifest(spec: list[dict]) -> str:
    lines = [
        ...
        f"*Project home (updates, the full TOS): {REPO_URL}*",
        "",
    ...
    lines += [
        "",
        "## Other ways to get this data (same information, different packaging)",
        "",
        f"- **`{ONEFILE}`** — the whole pack as ONE file. For ChatGPT **Free** (Projects hold only"
        " a handful of files there) or anyone who wants a two-download setup (this + TOS-skills.md)."
        " Tradeoff: the 11 separate files give the assistant better file-search granularity, so use"
        " them when your plan allows.",
        f"- **`{ZIPFILE_NAME}`** — all 11 files in one download; unzip it, then add the files to"
        " your Project.",
        "",
        "## Freshness",
        "",
...
def build() -> int:
    ...
    manifest_text = _manifest(spec)
    (PACK_DIR / "MANIFEST.md").write_text(manifest_text, encoding="utf-8")
    written.append("MANIFEST.md")
    n_project = len(written)  # the files a teacher puts in a Project
    # packaging alternates (NOT Project-slot files): one-file pack + zip
    (PACK_DIR / ONEFILE).write_text(_onefile(spec, manifest_text), encoding="utf-8")
    (PACK_DIR / ZIPFILE_NAME).write_bytes(_zip_bytes(spec))
    written += [ONEFILE, ZIPFILE_NAME]
    ...
    print(f"Reference pack: {n_project} Project files + {ONEFILE} + {ZIPFILE_NAME}, "
          f"{total/1e6:.1f} MB -> {PACK_DIR.relative_to(ROOT)}/")
    print(f"Project math: {n_project} pack + TOS-skills.md + your profile = {n_project + 2} of "
          f"~20 slots (Plus/Pro) — or onefile + TOS-skills.md + profile = 3 (fits Free)")
    if n_project > MAX_FILES:
        _fail(f"pack exceeds {MAX_FILES} files ({n_project}) — curate, don't dump")
    return 0
...
def check() -> int:
    spec = _pack_spec()
    problems = []
    names = {e["dest"] for e in spec} | {"MANIFEST.md", ONEFILE, ZIPFILE_NAME}
    ...
    # packaging alternates: rebuild-and-compare
    manifest_text = _manifest(spec)
    for name, rebuilt in ((ONEFILE, _onefile(spec, manifest_text).encode()),
                          (ZIPFILE_NAME, _zip_bytes(spec))):
        p = PACK_DIR / name
        if not p.exists():
            problems.append(f"missing {name}")
        elif hashlib.sha256(rebuilt).hexdigest() != _sha(p):
            problems.append(f"drift: {name} != rebuild from pack")
    extra = [p.name for p in PACK_DIR.iterdir() if p.name not in names]
    if extra:
        problems.append(f"unexpected files in pack: {extra}")
    n_project = len([p for p in PACK_DIR.iterdir() if p.name not in (ONEFILE, ZIPFILE_NAME)])
    if n_project > MAX_FILES:
        problems.append(f"pack has {n_project} Project files (max {MAX_FILES})")
    if problems:
        print("PACK DRIFT/CITATION REPORT:")
        for p in problems:
            print(f"  FAIL  {p}")
        return 1
    print(f"OK — {n_project} Project files (+ onefile + zip), sha256 matches sources, "
          f"every row cited, <= {MAX_FILES} files.")
    return 0
