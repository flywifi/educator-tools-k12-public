...
def parse_docx(text: str, code_re: str):
    ...
    for i, ln in enumerate(lines):
        ...
        for j in range(i + 1, min(i + 6, len(lines))):
            ...
            if not s:
                continue
            if line_code.match(lines[j]) or STOP.match(s):
                break
            stmt = s
            break
        seen.add(code)
        g, strand = info(code)
        out.append({"code": code, "grade": g, "strand": strand, "type": classify(code), "statement": stmt})
    return out
...
def main() -> int:
    ...
    for subj, (rel, code_re, fmt) in SUBJECTS.items():
        ...
        if not src.exists():
            ...
            continue
        text = doc_text(src) if fmt == "doc" else docx_text(src)
        ...
        index["subjects"][subj] = {"file": f"data/{subj}.json", "format": fmt, "count": len(entries),
                                   "benchmarks": t["benchmark"], "access_points": t["access_point"], "practices": t["practice"]}
        print(f"  {subj:16} {len(entries):5} codes  (benchmark {t['benchmark']}, AP {t['access_point']}, practice {t['practice']}, {fmt})")
