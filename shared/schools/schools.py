...
Loads district school/program indexes from shared/schools/<district>/schools.json and answers questions
...
Usage:
  python3 shared/schools/schools.py --list                          # all schools (all districts)
  python3 shared/schools/schools.py --district ocps --level MS      # filter
  python3 shared/schools/schools.py --magnet                        # schools with magnet/choice programs
  python3 shared/schools/schools.py --status closed                 # e.g. recently closed/consolidated
  python3 shared/schools/schools.py --find "high school"            # name/MSID lookup
  python3 shared/schools/schools.py --stats                         # coverage + counts (+ completeness)
"""
...
def main(argv) -> int:
    ...
    if not _districts():
        print(json.dumps({"status": "error", "detail": "no district indexes under shared/schools/*/schools.json"}))
        return 1
    if a.stats:
